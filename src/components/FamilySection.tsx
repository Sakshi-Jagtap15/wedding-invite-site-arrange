...
const FamilySection = () => {
  ...
  const isGroomFirst = invitation?.primary_side === 'groom';

  const brideBlock = (
    <div className={`flex-1 text-center px-8 py-10 ${isGroomFirst ? 'slide-right' : 'slide-left'}`}>
  ...
  const groomBlock = (
    <div className={`flex-1 text-center px-8 py-10 ${isGroomFirst ? 'slide-left' : 'slide-right'}`}>
    ...
    <section className="min-h-screen flex items-center justify-center bg-ivory mandala-bg py-24">
      <div className="max-w-5xl mx-auto px-6 w-full">
        ...
        <div className="flex flex-col md:flex-row items-start justify-center gap-0 md:gap-0">
          ...
          {isGroomFirst ? groomBlock : brideBlock}

          {/* Connective Group: wrapper for the vertical (Desktop) and horizontal (Mobile) dividers */}
          <>
          ...
          {isGroomFirst ? brideBlock : groomBlock}
